...
ex_3_1 %>% 
  ggplot(aes(x = DATE)) +
    ...
    annotate(geom = "rect", xmin = as_date("2019-05-20"), 
             xmax = as_date("2020-01-10"), ymin = 0, ymax = 2.55, 
             alpha = 0.1) + 
    annotate(geom = "text",
...
ex_3_2 %>% 
  ggplot(aes(x = Year_2019, y = Rate)) +
    ...
    annotate(geom = "segment", 
             x = 13, xend = 4.5, 
             y = 0.0075, yend = 0.0075,
             alpha = 0.3, fill = "grey60", arrow=arrow()) +
    annotate(geom = "segment", 
             x = 13, xend = 8.5, 
             y = 0.0045, yend = 0.0045,
             alpha = 0.3, fill = "grey60", arrow=arrow()) +
    scale_y_continuous(labels = percent_format(accuracy = 0.1),
                       breaks = seq(0, 0.01, 0.001),
                       limits = c(0, 0.01)) +
...
Show the best way to plot a promotion: 
...
ex_4_2 %>% 
  ggplot(aes(x = reorder(Brands, -perc), y = perc)) + 
    ...
    coord_flip() +
    geom_text(aes(x = reorder(Brands, -perc), y = 0.005, 
                  label = if_else(perc < 0, Brands, NULL)), 
              hjust = 0, size = 3, col = "grey65") + 
    geom_text(aes(x = reorder(Brands, -perc), y = -0.005, 
                  label = if_else(perc > 0, Brands, NULL)), 
              hjust = 1, size = 3, col = "grey65") + 
    scale_y_continuous(labels = percent_format(accuracy = 1), 
    ...
    theme(
      ...
      axis.text.x = element_text(colour = "grey65", size = 10), 
      axis.ticks.x = element_line(linetype = 1, colour = "grey65"), 
...
ex_4_2 %>% 
  ggplot(aes(x = reorder(Brands, -perc), y = perc)) + 
    geom_col(fill = "grey65") + 
    geom_col(aes(y = if_else(Brands %in% c("Lifestyle", 
                                           "Diet Lifestyle", 
                                           "Lifestyle Plus"), 
                             perc, NULL)), fill = "black") +
    coord_flip() +
    geom_text(aes(x = reorder(Brands, -perc), y = 0.005, 
                  label = if_else(perc < 0, Brands, NULL)), 
              hjust = 0, size = 3, col = "grey65") + 
    geom_text(aes(x = reorder(Brands, -perc), y = 0.005, 
                  label = if_else(perc < 0 & 
                                    Brands %in% c("Lifestyle", 
                                                  "Diet Lifestyle", 
                                                  "Lifestyle Plus"), 
                                  Brands, NULL)), 
              hjust = 0, size = 3, col = "black") + 
    geom_text(aes(x = reorder(Brands, -perc), y = -0.005, 
                  label = if_else(perc > 0, Brands, NULL)), 
              hjust = 1, size = 3, col = "grey65") + 
    scale_y_continuous(labels = percent_format(accuracy = 1), 
                       ...
                       limits = c(-0.2, 0.2), 
                       position = "bottom") +
    labs(title = "Cat food brands: Lifestyle line brands declined", 
         subtitle = "YEAR-OVER-YEAR % CHANGE IN SALES VOLUME ($)\n") + 
    theme_void() + 
    theme(
      title = element_text(colour = "grey20"),
      plot.subtitle = element_text(size = 10),
...
Focus on factors in increasing values: 

```{r}
rel_brands <- c("Lifestyle", "Diet Lifestyle", "Lifestyle Plus", "Feline Gold", 
                "Feline Platinum", "Feline Instinct", "Feline Pro", 
                "Feline Royal", "Feline Focus", "Feline Grain Free", 
                "Feline Silver")

ex_4_2 %>% 
  ggplot(aes(x = reorder(Brands, -perc), y = perc)) + 
    geom_col(fill = "grey65") + 
    geom_col(aes(y = if_else(Brands %in% rel_brands, 
                             perc, NULL)), fill = "dodgerblue4") +
    coord_flip() +
    geom_text(aes(x = reorder(Brands, -perc), y = 0.005, 
                  label = if_else(perc < 0, Brands, NULL)), 
              hjust = 0, size = 3, col = "grey65") + 
    geom_text(aes(x = reorder(Brands, -perc), y = 0.005, 
                  label = if_else(Brands %in% c("Lifestyle", 
                                                "Diet Lifestyle", 
                                                "Lifestyle Plus"), 
                                  Brands, NULL)), 
              hjust = 0, size = 3, col = "dodgerblue4") + 
    geom_text(aes(x = reorder(Brands, -perc), y = -0.005, 
                  label = if_else(perc > 0, Brands, NULL)), 
              hjust = 1, size = 3, col = "grey65") + 
    geom_text(aes(x = reorder(Brands, -perc), y = -0.005, 
                  label = if_else(Brands %in% c("Feline Gold", 
                "Feline Platinum", "Feline Instinct", "Feline Pro", 
                "Feline Royal", "Feline Focus", "Feline Grain Free", 
                "Feline Silver"), Brands, NULL)), 
              hjust = 1, size = 3, col = "dodgerblue4") + 
    scale_y_continuous(labels = percent_format(accuracy = 1), 
                       breaks = seq(-0.2, 0.2, 0.05), 
                       limits = c(-0.2, 0.2), 
                       position = "bottom") +
