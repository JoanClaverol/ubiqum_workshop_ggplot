...
ex_3 %>% 
  pivot_longer(cols = -DATE, names_to = "names", values_to = "values") %>%
  ggplot(aes(x = DATE, y = values)) +
    geom_col(aes(fill = names), position = "dodge") +
    scale_fill_manual(values = used_colors) + 
    scale_y_continuous(labels = number_format(big.mark = ".")) +
    labs(title = "Demand and capacity by Month\n") + 
    ylab("UNITS") + xlab("MONTH") +
    theme_void() +
    theme(
      legend.position = "top",
      legend.justification = "left",
...
ex_3_ready %>% 
  ggplot(aes(x = DATE, y = values)) +
    geom_line(aes(group = DATE), color = "gray") +
    geom_point(aes(color = names, group = names)) +
    geom_text(aes(label = if_else(
      DATE == "2020-03" & names == "DEMAND", values, NULL)
      ), 
      col = used_colors[2], hjust = 0.5, vjust = -0.5) +
...
ex_3 %>% 
  mutate(UNMET_DEMAND = DEMAND - CAPACITY, 
         GROUP = "1") %>% 
  # pivot_longer(cols = -DATE, names_to = "names", values_to = "values") %>%
  ggplot(aes(x = DATE, y = UNMET_DEMAND)) +
    geom_line(aes(group = GROUP), color = "dodgerblue4") +
    geom_point(color = "dodgerblue4") +
    scale_y_continuous(labels = number_format(big.mark = "."), breaks = seq(0, 30000, 5000)) +
    labs(title = "Unmet demand by Month\n") + 
    ylab("UNITS") + xlab("MONTH") +
    theme_void() +
    theme(
      legend.position = "top",
